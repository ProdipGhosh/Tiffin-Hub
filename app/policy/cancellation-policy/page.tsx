              ...
              return (
                <Link key={link.href} href={link.href}
                  className={`px-4 py-1.5 text-sm font-medium rounded-lg whitespace-nowrap transition-all duration-150 ${
                    isActive ? "bg-orange-50 text-orange-600 border border-orange-200" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                  }`}
                >
                  {link.label}
                </Link>
      ...
      <div className="flex flex-1" style={{ height: "calc(100vh - 56px)" }}>
        <aside className="hidden lg:flex flex-col w-52 shrink-0 bg-white border-r border-gray-200">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-100">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Guidelines &amp; Policies</p>
          </div>
          <ul className="py-2">
            {policyLinks.map((link) => {
              const isActive = pathname === link.href;
                ...
                <li key={link.href}>
                  <Link href={link.href}
                    className={`flex items-center gap-3 px-4 py-3 text-sm transition-all duration-150 ${
                      isActive ? "bg-orange-50 text-orange-600 font-semibold border-l-4 border-orange-500" : "text-gray-700 hover:bg-gray-50 hover:text-gray-900 border-l-4 border-transparent"
                    }`}
                  >
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-white">
          ...
          <div className="flex items-center gap-3 px-5 py-2 border-b border-gray-100 shrink-0">
            ...
            <div className="leading-tight">
              <p className="text-sm font-semibold text-gray-800">Tiffin Hub Cancellation Policy v1.0</p>
              <p className="text-xs text-gray-400">PDF Document</p>
            </div>
            <a href="/cancellation-policy.pdf" download
          ...
          <div className="flex-1 overflow-hidden relative bg-white">
            <iframe
              src="/cancellation-policy.pdf#toolbar=0&navpanes=0&scrollbar=1&zoom=page-width"
              title="Tiffin Hub Cancellation Policy"
              style={{
                position: "absolute",
                top: 0,
                left: "-60px",
